...
class Rooms {
                ...
                layout: {
                    ...
                    rows: [
                        ...
                        } ,
                        {
                            id: `2nd-row` ,
                            columns: [
                                { id: `inlineInput` ,widthPercent: 100, height: 2} ,
                            ]
                        }
                    ]
                }
    ...
    GenerateChannels = (channel) => {
        ...
        if(channel.type){
            ...
            if(channel.type === 'channel'){
                ...
                let chanServer = this.channelz.itemsDef.filter(e => e.value?.type === 'server' && e.value?.owner === channel.owner)[0]
                let serverChannelsNumber = this.channelz.itemsDef.filter(e => e.value?.type === 'channel' && e.value?.owner === channel.owner).length
                let indexOfServer = this.channelz.itemsDef.indexOf(chanServer)
                if(chanServer){
    ...
    DestroyChannel = (leftchanID) => {
        let newlist = this.channelz.itemsDef.filter(e => e.id !== leftchanID)
