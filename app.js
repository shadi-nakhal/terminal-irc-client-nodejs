        ...
        if (chanbutt && inputmsg) {
            ...
            if (connection?.identity === chanbutt.owner && value) {
                ...
                if (chanbutt.type === "server") {
                    if (inputmsg.command) {
                        if (Settings[chanbutt.owner].displayRaw) {
                            Settings[chanbutt.owner].raw += `${data}\r\n`;
                        } else {
                            Settings[chanbutt.owner].status += `${data}\r\n`;
                        }
                    }
                    Update();
                ...
                if (chanbutt.type === "private") {
                    if (!inputmsg.command) {
                        Settings[chanbutt.owner].private[chanbutt.name].logs += `^C${EscapeCarets(nickname)}^::${EscapeCarets(
                            data
                        )}\r\n`;
                    }
